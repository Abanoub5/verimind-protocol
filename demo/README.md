...
The attribution bridge was executed and verified during documentation work because it requires no network access. The full `run-demo.js` Hardhat script requires `npm install`, which depends on network access and was not available in the environment in which this repository was authored. See the top-level test/compilation report for details.
