...
def top_k_attribution(
    ...
    scored = [(c.address, cosine_similarity(query_embedding, c.embedding)) for c in creators]
    ...
    return [(addr, score, b) for (addr, _), score, b in zip(top, softmax_scores, bps)]
...
if __name__ == "__main__":
    ...
    query = [0.9, 0.1, 0.3]
    dataset = [
        Creator(address="0xCreatorA...", embedding=[0.88, 0.12, 0.31]),
        Creator(address="0xCreatorB...", embedding=[0.10, 0.95, 0.05]),
        Creator(address="0xCreatorC...", embedding=[0.50, 0.50, 0.50]),
    ]

    result = top_k_attribution(query, dataset, k=3, tau=0.1)
    ...
    print(f"  sum of bps = {sum(b for _, _, b in result)} (must equal 10000)")
